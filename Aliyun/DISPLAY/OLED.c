...
void OLED_I2C_Init(void)
{
    ...
    RCC_AHB1PeriphClockCmd(IIC_SCL_CLOCK, ENABLE);           //打开SCL的时钟
    RCC_AHB1PeriphClockCmd(IIC_SDA_CLOCK, ENABLE);           //打开SDA的时钟
...
//设置SDA引脚为输出模式
void IIC_SetSDAOutput(void)
{
    GPIO_InitTypeDef      GPIO_InitStructure;
    
    RCC_AHB1PeriphClockCmd(IIC_SDA_CLOCK, ENABLE);
...
{      	
	...
	for (i = 0; i < 16; i++)
	{
		OLED_WriteData(OLED_CH8x16[Char][i]);			//显示上半部分内容
	}
	OLED_SetCursor((Line - 1) * 2 + 1, (Column - 1) * 16);	//设置光标位置在下半部分
	for (i = 0; i < 16; i++)
	{
		OLED_WriteData(OLED_CH8x16[Char ][i +16]);		//显示下半部分内容
	}
}
